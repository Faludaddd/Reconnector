    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(spacing: appState.compactMode ? 12 : 20) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(spacing: 12) {
                            Button(role: .destructive) { showingRestartAlert = true } label: { Label("Force Restart Roblox", systemImage: "arrow.clockwise.circle.fill").frame(maxWidth: .infinity) }.buttonStyle(.borderedProminent).controlSize(.large)
                            Button { Task { await appState.fetchScreenshot(); showingScreenshot = true } } label: { Label("Take Screenshot", systemImage: "camera.fill").frame(maxWidth: .infinity) }.buttonStyle(.bordered).controlSize(.large)
                            Button { Task { try? await APIClient(ipAddress: appState.ipAddress, authToken: appState.authToken).activateBlackScreen() } } label: { Label("Black Screen", systemImage: "moon.fill").frame(maxWidth: .infinity) }.buttonStyle(.bordered).controlSize(.large)
                            Button { Task { try? await APIClient(ipAddress: appState.ipAddress, authToken: appState.authToken).clearAntiLoop() } } label: { Label("Clear Anti-Loop", systemImage: "arrow.uturn.left.circle").frame(maxWidth: .infinity) }.buttonStyle(.bordered).controlSize(.large)
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 4) { Text("Status"); Text(appState.status?.watchdog_enabled == true ? "Active" : "Disabled").foregroundColor(appState.status?.watchdog_enabled == true ? .green : .red).fontWeight(.medium) }
                            Spacer()
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        HStack(spacing: 12) {
                            ForEach([0, 50, 100], id: \.self) { level in
                                Button("\(level)%") { Task { try? await APIClient(ipAddress: appState.ipAddress, authToken: appState.authToken).setBrightness(level: level) } }
                                .buttonStyle(.bordered).tint(appState.status?.brightness == level ? appState.accentColor : .gray).frame(maxWidth: .infinity)
                            }
                        }
                    }
                    .padding(20).background(Color(UIColor.secondarySystemBackground)).cornerRadius(20)
            ...
            .alert("Force Restart?", isPresented: $showingRestartAlert) {
                ...
                Button("Restart", role: .destructive) { Task { try? await APIClient(ipAddress: appState.ipAddress, authToken: appState.authToken).restart() } }
            } message: { Text("This will force-stop Roblox and relaunch it. Are you sure?") }
